...
def _create_run_dir_local(submit_config: SubmitConfig) -> str:
    ...
    print('debug: run_dir is {}'.format(run_dir))
...
def _create_run_dir_set(submit_config: SubmitConfig) -> str:
    """Create a new run dir with increasing ID number at the start."""
    run_dir_root = get_path_from_template(submit_config.run_dir_root, PathType.AUTO)
    ...
    if not os.path.exists(run_dir):
        print(f'Do you want to run with {submit_config.run_name}?')
        os.makedirs(run_dir)

    return run_dir

def _find_latest_ckpt(run_dir: str, total_kimg: int) -> str:
    """Find the latest checkpoint inthe logging dir"""
    cur_index = 0
    ckpt = ''
    for item in os.listdir(run_dir):
        if 'pkl' in item and 'network-snapshot' in item:
            try:
                index = item.split('-')[2]
...
def run_wrapper(submit_config: SubmitConfig) -> None:
    ...
    if is_local:
        ...
        logger = util.Logger(file_name=os.path.join(submit_config.run_dir, "log_{}.txt".format(time_str)), file_mode="w", should_flush=True)
...
def submit_run(submit_config: SubmitConfig, run_func_name: str, **run_func_kwargs) -> None:
    ...
    submit_config.task_name = "{}".format(submit_config.run_name)
...
def submit_run_flexible(submit_config: SubmitConfig, run_func_name: str, **run_func_kwargs) -> None:
    """Create a run dir, gather files related to the run, copy files to the run dir, and launch the run in appropriate place."""
    submit_config = copy.deepcopy(submit_config)
    ...
    #--------------------------------------------------------------------
    #host_run_dir = _create_run_dir_local(submit_config)
    host_run_dir = _create_run_dir_set(submit_config)

    # enumerating the existing pkl, and load the latest.
    total_kimg = run_func_kwargs['total_kimg']
    resume_pkl, cur_kimg = _find_latest_ckpt(host_run_dir, total_kimg)
    print(f'resume_pkl after creating dir is {resume_pkl}, cur_kimg is {cur_kimg}')
    if resume_pkl != '':
        resume_pkl = os.path.join(host_run_dir, resume_pkl)
    else:
        resume_pkl = None
    submit_config.run_func_kwargs['resume_pkl'] = resume_pkl
    submit_config.run_func_kwargs['resume_kimg'] = cur_kimg
