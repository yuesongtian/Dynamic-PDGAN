...
class Network:
    ...
    def copy_specific_vars_from(self, src_net: "Network", var_name: "Name of variable") -> None:
        """Copy the values of var_name variables from the given network, including sub-networks."""
        for name in self.vars.keys():
            print(f'G var is {name}')
        for name in src_net.vars.keys():
            print(f'G_bar var is {name}')
        names = [var_name]
        tfutil.set_vars(tfutil.run({self.vars[name]: src_net.vars[name] for name in names}))
    ...
    def run_dlatents(self,
            *in_arrays: Tuple[Union[np.ndarray, None], ...],
            ...
            assume_frozen: bool = False,
            **dynamic_kwargs) -> Union[np.ndarray, Tuple[np.ndarray, ...], List[np.ndarray]]:
        """Run this network for the given NumPy array(s), and return the output(s) as NumPy array(s), including images and dlatents.
            ...
            with tfutil.absolute_name_scope(self.scope + "/_Run"), tf.control_dependencies(None):
                ...
                for gpu in range(num_gpus):
                    with tf.device("/gpu:%d" % gpu):
                        ...
                        assert len(in_gpu) == self.num_inputs
                        out_gpu, w = net_gpu.get_output_for(*in_gpu, return_as_list=True, **dynamic_kwargs)
                ...
                with tf.device("/cpu:0"):
                    out_expr = [tf.concat(outputs, axis=0) for outputs in zip(*out_split)]
                    out_w_expr = [tf.concat(ws, axis=0) for ws in zip(*w_split)]
                    self._run_cache[key] = in_expr, out_expr, out_w_expr

        # Run minibatches.
        in_expr, out_expr, out_w_expr = self._run_cache[key]
        print(f'in_expr is {in_expr}, out_expr is {out_expr}, w_expr is {out_w_expr}')
        out_arrays = [np.empty([num_items] + expr.shape.as_list()[1:], expr.dtype.name) for expr in out_expr]
        out_w_arrays = [np.empty([num_items] + expr.shape.as_list()[1:], expr.dtype.name) for expr in out_w_expr]
        ...
        for mb_begin in range(0, num_items, minibatch_size):
            ...
            mb_num = mb_end - mb_begin
            mb_in = [src[mb_begin : mb_end] if src is not None else np.zeros([mb_num] + shape[1:]) for src, shape in zip(in_arrays, self.input_shapes)]
            mb_out, w_out = tf.get_default_session().run([out_expr, out_w_expr], dict(zip(in_expr, mb_in)))

            #for dst, src in zip(out_arrays, mb_out):
            #    dst[mb_begin: mb_end] = src

            for dst, dst_w, src, w in zip(out_arrays, out_w_arrays, mb_out, w_out):
                dst[mb_begin: mb_end] = src
                dst_w[mb_begin: mb_end] = w
        ...
        if not return_as_list:
            out_arrays = out_arrays[0] if len(out_arrays) == 1 else tuple(out_arrays)
        return out_arrays, out_w_arrays
    ...
    def cal_flops(self, title: str = None, hide_layers_with_no_params: bool = False) -> None:
        """Print a summary table of the network structure."""
        rows = [[title if title is not None else self.name, 'weight_shape', "FLOPs"]]
        rows += [["---"] * 3]
        total_flops = 0
        ...
        for layer_name, layer_output, layer_trainables in self.list_layers():
            ...
            # Calculate FLOPs
            if 'Dense' in layer_name and num_params > 0:
                num_flops = weights[0].shape[0] * weights[0].shape[1]
                total_flops += num_flops
            elif num_params > 0:
                print(f'weights shape is {weights[0].shape}, output shape is {layer_output.shape}')
                num_flops = weights[0].shape[0] * weights[0].shape[1] * weights[0].shape[2] * layer_output.shape[2] * layer_output.shape[3] * layer_output.shape[1] + layer_output.shape[2] * layer_output.shape[3] * layer_output.shape[1]   # K_H * K_W * C_in * H * W * C_out + H * W * C_out
                total_flops += num_flops

            if not hide_layers_with_no_params or num_params != 0:
                flops_str = str(num_flops) if len(weights) >= 1 else "-"
                rows += [[layer_name, str(weights[0].shape) if len(weights) > 0 else "---", flops_str]]

        rows += [["---"] * 3]
        rows += [["Total", '---', str(total_flops)]]
