...
def upsample_conv_2d_woLP(x, w, k=None, factor=2, gain=1, data_format='NCHW', impl='cuda'):
    r"""Simple tf.nn.conv2dTranspose`.
    ...
    if data_format == 'NCHW':
        stride = [1, 1, factor, factor]
        output_shape = [_shape(x, 0), outC, (_shape(x, 2) - 1) * factor + convH -1 , (_shape(x, 3) - 1) * factor + convW - 1]
        num_groups = _shape(x, 1) // inC
    else:
        stride = [1, factor, factor, 1]
        output_shape = [_shape(x, 0), (_shape(x, 1) - 1) * factor + convH, (_shape(x, 2) - 1) * factor + convW, outC]
    ...
    # Execute.
    x = tf.nn.conv2d_transpose(x, w, output_shape=output_shape, strides=stride, padding='SAME', data_format=data_format)
    return x
