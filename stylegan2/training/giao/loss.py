...
def D_logistic_r1_dg(G, G_bar, D, opt, training_set, minibatch_size, reals, labels, gamma=10.0, alpha=0.3):
    _ = opt, training_set
    ...
    DG_term = autosummary('Loss/DG/D_term', DG_term)
    loss = tf.nn.softplus(fake_scores_out) + alpha * DG_term # -log(1-sigmoid(fake_scores_out))
    loss += tf.nn.softplus(-real_scores_out) # -log(sigmoid(real_scores_out)) # pylint: disable=invalid-unary-operand-type
    autosummary('Loss/D_loss', loss[0])
...
def G_logistic_ns_pathreg(G, D, opt, training_set, minibatch_size, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2.0):
    ...
    fake_scores_out = D.get_output_for(fake_images_out, labels, is_training=True)
    loss = tf.nn.softplus(-fake_scores_out) # -log(sigmoid(fake_scores_out))
    autosummary('Loss/g_loss', loss[0])
    print(f'output shape is {G.output_shape}')
...
def G_dg_pathreg(G, D, D_bar, opt, training_set, minibatch_size, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2.0, alpha=0.3):
    _ = opt
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    labels = training_set.get_random_labels_tf(minibatch_size)
    fake_images_out, fake_dlatents_out = G.get_output_for(latents, labels, is_training=True, return_dlatents=True)
    ...
    fake_scores_out = D.get_output_for(fake_images_out, labels, is_training=True)
    DG_term = tf.nn.softplus(-fake_scores_out_bar)
    DG_term = autosummary('Loss/DG/G_term', DG_term)
    loss = tf.nn.softplus(-fake_scores_out) + alpha * DG_term   # -log(sigmoid(fake_scores_out))
