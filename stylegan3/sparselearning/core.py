...
class CosineDecay(object):
    def __init__(self, death_rate, T_max, eta_min=0.005, last_epoch=-1, init_step=0):
...
class Masking(object):
    def __init__(self, G_optimizer=False, D_optimizer=False,  death_rate_decay=False, death_rate=0.3, death='magnitude', G_growth='gradient', D_growth='random', redistribution='momentum', device=None, resume_steps=0, **config):
        ...
        print(f'death_rate_decay is {death_rate_decay}')
    ...
    def init(self, mode='ERK', density=0.05, densityG=0.5, erk_power_scale=1.0, customer_Gmasks=None, customer_Dmasks=None):
        ...
        if mode == 'uniform':
            for name, weight in self.G_model.named_parameters():
                ...
                self.G_masks[name][:] = (torch.rand(weight.shape) < self.G_density).float().data.to(self.device)
            for name, weight in self.D_model.named_parameters():
                if name not in self.D_masks: continue
                self.D_masks[name][:] = (torch.rand(weight.shape) < self.D_density).float().data.to(self.device)
        elif mode == 'pruning':
            ...
            for name, weight in self.G_model.named_parameters():
                if name not in self.G_masks: continue
                self.G_masks[name][:] = (weight != 0.0).float().data.to(self.device)
            for name, weight in self.D_model.named_parameters():
                if name not in self.D_masks: continue
                self.D_masks[name][:] = (weight != 0.0).float().data.to(self.device)
        elif mode == 'ERK':
        ...
        for name, weight in self.G_masks.items():
            ...
            print(f'in init_G, device is {weight.device}, self.device is {self.device}')
        ...
        for name, weight in self.D_masks.items():
            ...
            print(f'in init_D, device is {weight.device}, self.device is {self.device}')
    ...
    def add_module(self, G_model, D_model, densityG=0.5 , density=0.5, sparse_init='ERK', customer_Gmasks=None, customer_Dmasks=None):
        ...
        for name, tensor in self.G_model.named_parameters():
            if len(tensor.size()) == 4:
                self.G_masks[name] = torch.zeros_like(tensor,  requires_grad=False).to(self.device)

        for name, tensor in self.D_model.named_parameters():
            if len(tensor.size()) == 4:
                self.D_masks[name] = torch.zeros_like(tensor,  requires_grad=False).to(self.device)
        ...
        if 'G' in apply_mode:
            ...
            for name in model_para:
                if name in self.G_masks:
                    self.G_masks[name].to(model_para[name].device)
        ...
        if 'D' in apply_mode:
            ...
            for name in model_para:
                if name in self.D_masks:
                    self.D_masks[name].to(model_para[name].device)
    ...
    def ERK_initialize_G(self, erk_power_scale):
        ...
        for name, mask in self.G_masks.items():
            ...
            self.G_masks[name][:] = (torch.rand(mask.shape) < density_dict[name]).float().data.to(self.device)
    ...
    def ERK_initialize_D(self, erk_power_scale):
        ...
        for name, mask in self.D_masks.items():
            ...
            self.D_masks[name][:] = (torch.rand(mask.shape) < density_dict[name]).float().data.to(self.device)

            total_nonzero += density_dict[name] * mask.numel()
        print(f"Overall sparsity of D {total_nonzero / total_params}")
    
    def synchronism_masks(self, mode='G'):
        if mode == 'G':
            for name in self.G_masks.keys():
                torch.distributed.broadcast(self.D_masks[name], src=0, async_op=False)
        elif mode == 'D':
            for name in self.D_masks.keys():
                torch.distributed.broadcast(self.D_masks[name], src=0, async_op=False)
