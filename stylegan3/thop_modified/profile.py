...
import training.networks_stylegan2 as st2
...
register_hooks = {
    ...
    st2.SynthesisLayer: count_synthesis,
    st2.FullyConnectedLayer: count_linear,
...
def profile(model: nn.Module, inputs, custom_ops=None, verbose=True):
    ...
    def add_hooks(m: nn.Module):
        ...
        if isinstance(m, st2.FullyConnectedLayer):
            m_type = st2.FullyConnectedLayer
            print(f'FullyConnected Layer, in hooks {m_type in register_hooks}')
        elif isinstance(m, st2.SynthesisLayer):
            m_type = st2.SynthesisLayer
            print(f'Synthesis Layer, in hooks {m_type in register_hooks}')
        else:
            m_type = 'Exception'
        ...
        if m_type in custom_ops:  # if defined both op maps, use custom_ops to overwrite.
            fn = custom_ops[m_type]
            if m_type not in types_collection and verbose:
                print("Customize rule %s() %s." % (fn.__qualname__, m_type))
        elif m_type in register_hooks:
            fn = register_hooks[m_type]
            if m_type not in types_collection and verbose:
                print("Register %s() for %s." % (fn.__qualname__, m_type))
        else:
            if m_type not in types_collection and verbose:
                print("Cannot find rule for %s. Treat it as zero Macs and zero Params." % m_type)
